...
##### 2.1.1.1. Workspace / カスタムセットホーム画面（Adobe Analytics）
...
後ほどこのガイドで、ワークスペースについて詳しく説明します。 セクション 2.1.2.1を参照
...
##### 2.1.1.2. ホーム画面インサイト（Google Analytics）
...
これはおそらく、Adobe Analytics になくて最も残念に思う機能の 1 つでしょう。 事前に作成されたホーム画面はありません。 ただし、上記のリストから必要なものをレプリケートしてランディング画面として設定するように、カスタムワークスペースを簡単に設定できます。 このトピックについては、後ほど詳しく説明します（またはAdobe Workspaceのセクション 2.1.2.1を参照）。
...
>[!TIP]
>
>遊んだり探検したりすることを恐れないでください。 箱の外で考える方法はたくさんあります。 さらに、作成したものが自分の考えを示しているかどうかを検証してください。 経験が役に立ちます。
...
##### 2.1.2.2. Google Analytics：ダッシュボード、カスタムレポートおよび保存済みレポート
...
ダッシュボードは、データのレポートレットを 1 つのビジュアライゼーションにまとめることができるレガシー製品でした。 ただし、Workspaceの機能（セクション 2.1.2.1）は操作が非常に簡単なので、この機能が廃止される前に再構築する必要があるレガシーレポートへのアクセスポイントとしてのみ存在します。
...
#### 2.2.1. Adobe Analytics Report Builder （Microsoft® Excel拡張機能）
...
AdobeのWorkspace（セクション 2.1.2.1）では、APIを使用してレポートを作成し、Workspaceでデバッグモードを有効にすると、使用された正確なAPI呼び出しが表示されます。 この方法を使用すれば、API 呼び出しをすばやく作成することができます。 Workspace を使用して取り込むデータを作成および検証すると、それらの API 呼び出しを使用してデータを独自の処理に出力できます。
...
残念ながら、これは私が使った経験のないGoogleツールです。 理論的には、これはAdobeのデータフィードに似ており、エンジニアリングチームがGoogle Analyticsアカウントから生データにアクセスできるようになります。
