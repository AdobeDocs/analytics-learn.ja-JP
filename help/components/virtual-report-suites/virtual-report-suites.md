---
title: Adobe Analyticsのバーチャルレポートスイート
description: Adobe Analyticsでは、ユーザー向けの「バーチャルレポートスイート」を作成できます。このレポートスイートでは、データをセグメント化し、ユーザー/グループを許可されたデータセグメントのみに制限します。 このビデオでは、仮想レポートスイートを作成および操作する方法を説明します。
feature: VRS
kt: 2325
role: User
level: Intermediate
last-substantial-update: 2023-10-18T00:00:00.000Z
exl-id: db1c229a-96a0-4ca0-8c4e-a04721564c7d
TQID: https://experienceleague.adobe.com/gu6lsITCAuvjP2SwxpxLl5RFADWl9ka3paiqcv2M7KA
product_v2:
  - id: e55547f1-a1ff-40c6-8978-026e40ab7fa4
feature_v2:
  - id: c153fd90-23e1-4614-81d3-3cc7571227f7
subfeature_v2:
  - id: b0a1f9d5-5795-42a3-a6d0-bd0e2748fd06
  - id: ef60b66e-5984-4336-ba72-6d978b1b6f87
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: b5a62a22-46f7-4f0d-b151-3fc640bef588
topic_v2:
  - id: d095671a-1355-40aa-8b5f-06c33c68080b
source-git-commit: 677e5a22dab92be7ff021c8410525b9091975aef
workflow-type: tm+mt
source-wordcount: 111
ht-degree: 9%

---

# Adobe Analyticsのバーチャルレポートスイート {#virtual-report-suites-in-adobe-analytics}

Adobe Analyticsでは、ユーザー向けの「バーチャルレポートスイート」を作成できます。このレポートスイートでは、データをセグメント化し、ユーザー/グループを許可されたデータセグメントのみに制限します。 このビデオでは、仮想レポートスイートを作成および操作する方法を説明します。 詳細なドキュメントについては、[仮想レポートスイートの概要](https://experienceleague.adobe.com/docs/analytics/components/virtual-report-suites/vrs-about.html?lang=ja){target="_blank"}を参照してください。

>[!VIDEO](https://video.tv.adobe.com/v/25412/?quality=12&learn=on)

