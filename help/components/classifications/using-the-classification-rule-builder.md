---
title: Adobe Analytics での分類ルールビルダーの使用
description: 分類ルールビルダーを使用して、Adobe Analytics でルールベースの分類を自動的に適用する方法を説明します。
feature: Classifications
topics: null
kt: 1936
role: User
level: Beginner
last-substantial-update: 2026-03-13T00:00:00Z
exl-id: 066d0ebf-1921-48be-8531-bdd766330825
source-git-commit: ef2db82030e0853badc174825fdafa5c261c8f5d
workflow-type: tm+mt
source-wordcount: '73'
ht-degree: 63%

---

# Adobe Analytics での [!UICONTROL 分類ルールビルダー] の使用 {#using-the-classification-rule-builder-in-adobe-analytics}

>[!WARNING]
>
>この分類ルールビルダーは非推奨となり、2027 年 2 月 28 日（PT）以降はアクセスできなくなります。 引き続き機能するには、分類セット ルールビルダー機能に切り替えてください。

[!UICONTROL 分類ルールビルダー] を使用して、Adobe Analytics でルールベースの [!UICONTROL 分類] を自動的に適用する方法を説明します。

>[!VIDEO](https://video.tv.adobe.com/v/3434374?captions=jpn&quality=12&learn=on)
