---
title: Experience Platform [!DNL tags]で購入イベントのルールを追加します
description: このビデオでは、 [!DNL tags] の製品文字列拡張機能を使用してサイトでの購入を追跡し、分析で収益、注文、単位の指標を取得する方法を説明します。
feature: Tags
topics: Development
kt: 3593
role: Developer
level: Beginner
exl-id: d5f29523-5726-4c49-8ce6-a21c0a7a6c73
TQID: https://experienceleague.adobe.com/XOKzWpVZ82-1IyM46qvoppcZ6dKaEBmKo4j08vF3F6Q
product_v2:
  - id: e55547f1-a1ff-40c6-8978-026e40ab7fa4
feature_v2:
  - id: b069d60e-95f3-44d6-95a8-ddc862a4bc38
  - id: b3f03848-ae12-48b2-8aab-cad18567eb32
subfeature_v2:
  - id: f1f1a2d4-0976-4881-b091-c2bb8de7ffac
role_v2:
  - id: ff6a42d2-313e-452e-93a6-792e4fad9ff8
level_v2:
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
source-git-commit: 677e5a22dab92be7ff021c8410525b9091975aef
workflow-type: tm+mt
source-wordcount: 78
ht-degree: 39%

---

# Experience Platform [!DNL tags]での購入イベントのルールを追加

このビデオでは、[!DNL Experience Platform Launch] の製品文字列[!UICONTROL 拡張機能]を使用して、サイトでの購入を追跡し、分析の[!UICONTROL 売上高]、[!UICONTROL 注文]、[!UICONTROL 数量]の[!UICONTROL 指標]を取得する方法を説明します。

>[!NOTE]
>
> Launch は **[!DNL tags]** になりました

>[!VIDEO](https://video.tv.adobe.com/v/31396/?captions=jpn&quality=12&learn=on)
