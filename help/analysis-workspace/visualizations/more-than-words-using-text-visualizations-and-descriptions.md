---
title: 単語以上 – Analysis Workspaceでのテキストビジュアライゼーションと説明の使用
description: プロジェクトでテキストのビジュアライゼーションと説明を使用して、エンドユーザー向けに洗練されたAnalysis Workspaceを作成する方法を説明します。
feature: Visualizations
role: User
level: Beginner
doc-type: Article
last-substantial-update: 2023-05-16T00:00:00.000Z
jira: KT-13267
thumbnail: KT-13267.jpeg
exl-id: e7792731-11eb-4186-a12f-0e5b2d0b7663
TQID: https://experienceleague.adobe.com/JFCys3PfiNrBoCkeNPLWD0rZB5qZYF15Uzy7fVySxyw
product_v2: id: e55547f1-a1ff-40c6-8978-026e40ab7fa4
feature_v2: id: b069d60e-95f3-44d6-95a8-ddc862a4bc38id: b0ca67c6-0a35-482c-ad91-baac1bcb26d6id: c153fd90-23e1-4614-81d3-3cc7571227f7
subfeature_v2: id: dcae653e-62c6-4cc8-84e6-ee110b848296
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2: id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2: id: e1e0219c-f879-479f-8427-888ed2a6e9c2
source-git-commit: 677e5a22dab92be7ff021c8410525b9091975aef
workflow-type: tm+mt
source-wordcount: 496
ht-degree: 1%

---

# 単語以上 – Analysis Workspaceでのテキストビジュアライゼーションと説明の使用

Adobe Analytics Analysis Workspaceを利用している企業は、データとデータビジュアライゼーションに重点を置くことが自然です。誰でも要約を入力できます？ しかし、テキストのビジュアライゼーションやビジュアライゼーションの説明など、Analysis Workspaceの機能を見落とすと、インサイトを価値のあるテキスト、画像、GIF、リンクと組み合わせる貴重な機会を逃す可能性があります。 データの意味を利用者に伝えるために参考文献やより多くのコンテキストを提供することで、データをより効果的かつ効果的なものにすることができます。

まず、次の機能について、必要な機能を確認しましょう。

- ビジュアライゼーションの説明を追加または変更するには、要素の上部領域を右クリックし、「説明を編集」リンクを選択します。

  ![text 01](assets/t01.png)


- 別のテキストパネルを追加するには、左側のナビゲーションバーのビジュアライゼーションメニューをクリックします。

  ![text 02](assets/t02.png)

このガイドでは、これらの2つの方法のいずれかを使用した例を示していますが、説明とテキストのビジュアライゼーションの両方で同様の追加を行うことができます。 また、フォントのピッチ、整列、カラーを調整し、箇条書きや自動番号リストを作成することもできます。

![text 03](assets/t03.png)

では始めましょう。 Analysis Workspace プロジェクトに追加する非常に有用なコンテキストの1つは、ハイパーリンクです。これには、レポートで詳細に説明されているページのURL、他のAnalysis Workspace プロジェクトへのリンク、外部レポートのコンテキストページなど、レポートを表示する際に役立つ可能性のあるその他のページへのリンクが含まれます。 この機能は、テキストの任意の部分を選択し、「リンク」アイコンを選択することでアクセスできます。

![text 04](assets/t04.png)

その結果、Analysis Workspace プロジェクトを表示しているすべてのユーザーが、オンラインまたは組織のイントラネットでアクセス可能なあらゆるページに、ワンクリックでアクセスできるようになります。

![text 05](assets/t05.png)

コンテンツをレポートに直接取り込む場合は、画像リンクを使用して、データに沿ってコンテンツを配置できます。

![text 06](assets/t06.png)

パブリック URLでアクセス可能な画像をレポートに追加できます。URLが&#x200B;*https*&#x200B;形式で、.png、.jpeg、.jpg、または.gif形式である限り、制限があるように聞こえますが、imgurやGIPHYなどの画像やGIFを共有するために使用されるオンラインツールでは、Share Linkを使用してWorkspaceにアクセス可能なファイルをアップロードする簡単な方法を提供できます。

その成果として、 データが直接参照しているweb ページをプロジェクトで表示できます。

![text 07](assets/t07.png)

プロジェクトでGIFを使用して、サイトのウォークスルー、サイト内の理想的なパス、アプリ上のタスクなどの動く画像を含めたり、レポートが&#x200B;***心を揺さぶる***&#x200B;状態になったことをチームに示したりすることもできます。

![text 08](assets/t08.png)

## 作成者

このドキュメントの作成者：

![ ダン・カミングス ](assets/text09.png)

McDonald’s Corporation、シニア製品エンジニアリング分析マネージャー、**Dan Cummings**

Adobe Analytics チャンピオン
